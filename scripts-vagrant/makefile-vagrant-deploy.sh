...
if releaseUploaded; then
    ...
    echo "Box ${BOX_PACKED} deployed at $(date)" >> logs/box-${BOX_VERSION}-${BOX_PACKED}-created
    echo "Box ${BOX_PACKED} deployment has taken ${duration}" >> logs/box-${BOX_VERSION}-${BOX_PACKED}-created
else
    exit 1
fi
