...
# load bash-tools bootstrap
# shellcheck source=../vendor/bash-tools/bash-framework/_bootstrap.sh
source "${ROOT_DIR}/vendor/bash-tools/bash-framework/_bootstrap.sh"
...
@test "Docker minimal version" {
  ...
  run Version::compare ${version:-1} ${MINIMAL_DOCKER_VERSION}
  [[ ${status} -ge 0 ]]
}
...
@test "Npm minimal version" {
  ...
  run Version::compare "${version:-1}" "${MINIMAL_NPM_VERSION}"
  [[ ${status} -ge 0 ]]
}
